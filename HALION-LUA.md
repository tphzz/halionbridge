...
Generated build scripts must distinguish assignments that are essential to a correct preset from optional decoration. For the built-in SFZ converter, sample-zone type, sample filename, root key, key range, and velocity range are required assignments: if HALion rejects any of them, the generated build script returns `ok = false` before saving. Optional assignments such as display names, sustain-loop parameters, amp velocity-to-level, and filter cutoff may be attempted defensively and logged as skipped when HALion rejects them. Generated SFZ scripts write the sample filename, root key, loop parameters, and optional tone parameters before the final key/velocity fields so audio-file sampler metadata cannot overwrite the intended SFZ mapping.
