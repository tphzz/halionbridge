...
Vendored third-party code is pulled in through Git submodules under `external/`. JUCE lives at `external/JUCE`, spdlog at `external/spdlog`, sfizz at `external/sfizz`, and CLI11 at `external/CLI11`. CLI11 is header-only and linked privately to `halionbridge_shared` and `halionbridge_static` for command-line parsing; it must not appear in public headers or converter public APIs.
...
Converter public headers live under `converters/*/include` and must remain free of JUCE, spdlog, and parser implementation types such as sfizz classes or CLI11 declarations. Converter modules describe the input format only; `sfz` means "SFZ to the standard halionbridge Lua build-directory shape" because the Lua destination is implied by halionbridge.
...
Top-level command classification and the built-in `build`, `remap-vstpresets`, and hidden build-worker option parsers are owned by the private `src/CliCommand.*` module and backed by CLI11. The public `Bridge::parseArguments()` and `Bridge::parseVstPresetRemapArguments()` functions remain stable wrappers around that private parser. Converter-specific argument parsing is still owned by each converter implementation and does not depend on CLI11.

Every public user-facing CLI invocation starts with the same `halionbridge <version>` header. Explicit help requests such as `halionbridge --help`, `halionbridge build --help`, `halionbridge init --help`, `halionbridge remap-vstpresets --help`, `halionbridge convert --help`, and converter-specific help such as `halionbridge convert sfz --help` print plain text to stdout and exit successfully. Command-line classification failures, syntax failures, missing required command arguments, and pre-runtime validation failures print plain diagnostics directly below the header instead of using the timestamped logger. Syntax failures append the narrowest relevant help text. Validation failures, such as nonexistent directories, missing build files, missing plugins, or nonexistent converter source paths, keep their focused diagnostic and do not dump usage. Hidden worker commands are exempt because their output is part of the parent/worker runtime protocol.

## Runtime Linkage

The release CLI itself is packaged as a single binary. It links against the internal `halionbridge_static` target, so the bridge implementation, embedded Lua/bootstrap assets, JUCE module code, CLI11 parser headers, and spdlog header-only logging are linked into `halionbridge.exe` on Windows and `halionbridge` on macOS. The release archives include tracked examples for users to run, but intentionally do not include `halionbridge.dll`, `libhalionbridge.dylib`, import libraries, static libraries, test executables, generated example output presets, or build directories.
...
The macOS release CLI is signed with hardened runtime plus `com.apple.security.cs.disable-library-validation` so the single-file host can load the installed HALion VST3 bundle at runtime. Omitting that entitlement can make the signed GitHub artifact fail with VST3 load or scan errors even though local unsigned developer builds load HALion successfully.
...
The workflow validates compilation, unit tests, and offline CLI parser/file tests only. The GitHub Actions test steps and CTest entries set `HALIONBRIDGE_FORBID_PLUGIN_INSTANTIATION=1`, and the bridge refuses to enter the VST3 load/instantiate path when that variable is enabled. GitHub-hosted tests must never install, launch, instantiate, or smoke-test HALion; HALion runtime testing still requires a licensed and activated local or self-hosted machine.
...
```text
halionbridge build <build-directory>
```

The built-in `sfz` converter accepts a source path that is either one `.sfz` file or a directory of `.sfz` files. Directory input processes only top-level `.sfz` files by default and includes nested files only when `--recursive` is passed; `--recursive` is rejected for file input. If the output directory is omitted, generated files are written beside the source `.sfz` file or flat into the source directory. If an output directory is supplied, generated files are written there instead. Each `.sfz` file currently generates one flat Lua build script in the build root; recursive source discovery does not create nested generated Lua. One generated `halionbridge_build.lua` lists all generated build entrypoint scripts in deterministic order. Converter-generated helper modules may be emitted beside build scripts and are treated as generated outputs for overwrite checks, but they are not listed in `halionbridge_build.lua`. `--overwrite` is required to replace existing generated files, including `halionbridge_build.lua`, `halionbridge-sfz.lua`, and generated entrypoint scripts. `--name <name>` is accepted only when exactly one `.sfz` file is converted. Output `.vstpreset` names use safe ASCII stems derived from source names, translate common musical accidentals such as `#`, `^`, and sharp/flat/natural symbols into readable words, avoid Windows device basenames, and append deterministic numeric suffixes such as `_002` only when case-insensitive collisions remain.
...
`converters/common` owns reusable converter infrastructure: converter registration, build-directory writing, Lua string escaping, overwrite checks, and deterministic LF output. Format-specific converter modules should pass generated Lua files to this common emitter instead of rewriting build-file logic. Generated Lua files have a role: `buildEntrypoint` files are written and listed in `halionbridge_build.lua`, while `helperModule` files are written but not listed. The emitter accepts only flat relative `.lua` filenames, rejects absolute paths, root paths, `.`/`..`, nested paths, non-`.lua` names, duplicate filenames, duplicate build-entrypoint module names, helper-only output, and reserved helper filenames used as build entrypoints. Validation and overwrite checks run before writing. File contents are written to same-directory transaction files first, then committed with backups for overwritten targets so a late filesystem failure can roll back without leaving a half-updated build directory.

`converters/sfz` owns all sfizz interaction. sfizz is an implementation dependency only; sfizz headers and types must not appear in converter APIs exposed outside implementation code. The v1 SFZ mapping creates one HALion layer preset per source SFZ and maps sample paths, sample playback range, key ranges, velocity ranges, root key, sustain-loop fields, static amp-envelope fields, static pitch/tuning fields, static gain/velocity/pan fields, key/velocity crossfade fields, and simple tone fields into the tested HALion sample-zone Lua pattern. Generated Lua treats zone type, sample filename, key range, velocity range, root key assignment, amp-envelope assignment, and emitted crossfade assignment as required; failure returns an unsuccessful build result before saving the preset. Optional naming, sample playback range, sustain-loop parameter assignment, sample oscillator level, amp velocity-to-level, amp pan, filter fields, and static pitch/tuning parameter assignment remain non-fatal and are logged by the generated build script when HALion rejects them. SFZ `offset` maps to HALion `SampleOsc.SampleStart`; SFZ inclusive `end=N` maps to HALion `SampleOsc.SampleEnd=N+1`, and the helper writes sample end before sample start so HALion does not clamp start markers against an unset end marker. For WAV-backed regions without explicit `end=`, the converter emits `sample_playback.natural_end` from the WAV frame count so the helper can initialize `SampleOsc.SampleEnd` before optional start or loop marker writes without treating the file-derived length as an authored SFZ end. SFZ `loop_mode=loop_continuous` maps to HALion sustain loop mode `1`, `loop_mode=loop_sustain` maps to HALion sustain loop mode `4`, and generated loop data writes `loop_end + 1` only to `SampleOsc.SustainLoopEndA`; loop assignment no longer truncates `SampleOsc.SampleEnd` to the loop end. Generated SFZ scripts write region progress only periodically so large SFZ files avoid thousands of progress-marker `savePreset()` calls. Static gain conversion maps SFZ `volume` in dB plus the verified `+7.8 dB` HALion compensation to per-zone `SampleOsc.Level`; generated layers disable velocity-setting inheritance and set `VelocityToLevelCurve=1`, which probe `030` verified as HALion's squared Main velocity curve for SFZ default `amp_veltrack=100`. `amp_veltrack` is clamped to SFZ's -100..100 percent range with a converter warning before being written to HALion `Amp Env.VelocityToLevel`; probe `032` keeps `amp_veltrack=200` as an out-of-range clamp regression. SFZ `pan` maps to zone `Amp.Pan`; probe `033` verified center, half-left/right, and full-left/right against sforzando, and changing HALion pan law made the match worse, so HALion's default pan law is intentionally left unchanged. SFZ key crossfades map `xfin_lokey/hikey` and `xfout_lokey/hikey` to HALion `Amp.CrossfadeKeyLo/Hi` edge widths, using an inclusive key width. SFZ velocity crossfades map `xfin_lovel/hivel` and `xfout_lovel/hivel` to `Amp.CrossfadeVelLo/Hi`, using high-minus-low velocity width; CC crossfades and non-default crossfade curves remain unsupported warnings. Probe `078` selected this parameter model, and probe `079` is the converter-backed regression using the normal calibrated level path. Static pitch conversion maps `pitch_keycenter` to `SampleOsc.Rootkey`, combines `transpose * 100 + tune` into `SampleOsc.Tune` with a -1200..1200 cent clamp, and maps `pitch_keytrack` to `Pitch.CenterKey` plus `Pitch.KeyFollow` with a -200..200 percent clamp. Static filter conversion is intentionally rough: supported sforzando-recognized LPF, HPF, BPF, BRF, `lsh`, `hsh`, and `peq` families map to HALion Classic filter type/mode/shape/cutoff/resonance values selected in probes `059`/`060`; `lpf_2p` keeps its earlier piecewise calibrated resonance table from probes `056`/`057`. State-variable `_sv` filter names and `pink` remain unsupported warnings rather than silent approximations.

Static pitch envelope conversion currently covers only the probe-verified simple `pitcheg_*` subset. `pitcheg_depth` is clamped to `-6000..6000` cents, converted to HALion `Pitch.EnvAmount` in semitones, and emitted as a required pitch-envelope assignment when nonzero. Zero-attack pitch envelopes start at level `1`; simple attack, hold, and sustain points are written directly. Positive `pitcheg_decay` values use the current probe-selected approximation of `0.30 * pitcheg_decay` with Lua envelope curve `-0.6`; probe `064` is the converter-backed regression. Advanced pitch-envelope behavior, including shape, CC, velocity, and flex-EG pitch targets, is reported as `unsupported-pitch-envelope` rather than silently approximated.

Static pitch LFO conversion covers one probe-verified simple pitch-targeted LFO per region. Numbered SFZv2 opcodes `lfoNN_pitch`, `lfoNN_freq`, `lfoNN_phase`, `lfoNN_delay`, `lfoNN_fade`, and selected `lfoNN_wave` values are preferred, while legacy `pitchlfo_depth`, `pitchlfo_freq`, `pitchlfo_phase`, `pitchlfo_delay`, and `pitchlfo_fade` use the same HALion mapping with the sine/default waveform. Depth is clamped to `-1200..1200` cents and normally maps to HALion modulation `Destination.Depth = pitch_depth / 20`; frequency maps directly to `LFO 1.Rate`, phase maps directly to `LFO 1.InitPhase = phase * 360`, and delay/fade seconds map to `LFO 1.Delay`/`LFO 1.FadeIn` milliseconds. Generated Lua sets `LFO 1.Trigger=1` so note starts retrigger the phase, then assigns `ModulationSource.lfo1` to a Pitch modulation row as a required operation. Probe `074` through `077` selected these SFZ-to-HALion wave mappings: `0` triangle to `WaveForm=1, Shape=25`, `1` sine/default to `WaveForm=0`, `3` square/pulse to `WaveForm=3, Shape=50` with a static `Pitch.Coarse`/`Pitch.Fine` center shift of `lfoNN_pitch / 2` cents and HALion internal `Destination.Depth = lfoNN_pitch * 0.03`, `6` saw up to `WaveForm=4`, `7` saw down to `WaveForm=2`, and ARIA `12` sample-and-hold to `WaveForm=6`. Multiple pitch LFOs, sub-waveforms, sync variants, dynamic CC/velocity mappings, and non-pitch LFO targets are reported as `unsupported-pitch-lfo` instead of being silently approximated. Probes `071` through `077` are the manual validation chain for this mapping.

Static filter envelope conversion covers the probe-verified simple `fileg_*` subset only when a supported static filter is also mapped. The converter emits HALion `Filter.EnvAmount` plus `Filter Env.EnvelopePoints` as required assignments. Attack-style envelopes map `fileg_depth` to `depth / 96`, use envelope peak and sustain level `0.72`, and preserve simple delay, attack, hold, sustain, and decay timing with the calibrated decay scale. Decay-only envelopes map `fileg_depth` to `depth / 48`, start from a depth-dependent level, and fall to zero over `0.30 * fileg_decay` with the probe-selected depth-dependent curve. Probe `070` is the converter-backed regression for the accepted `069` manual validation. Advanced filter-envelope behavior, including shape, CC, velocity, and flex-EG filter targets, is reported as `unsupported-filter-envelope` rather than silently approximated.

The SFZ converter-owned helper module lives at `converters/sfz/lua/halionbridge-sfz.lua` and is embedded into the converter at build time. The generated C++ header splits the embedded helper into small adjacent raw string literal chunks so MSVC does not reject the helper as an oversized string literal as the converter-owned Lua module grows. Converted SFZ output directories include it as a helper module named `halionbridge-sfz.lua`; it is not listed in `halionbridge_build.lua`. Generated SFZ entrypoint scripts require this helper, keep readable normalized region data blocks for sample playback, mapping, amp envelope, pitch envelope, pitch LFO, filter envelope, crossfade, loop, gain, filter, and pitch fields, and call the helper for HALion layer, sample-zone, region-application, and save behavior instead of duplicating assignment functions in every script. Helper version `1` exposes conservative capabilities and defensive HALion assignment helpers for the fields already covered by the current converter output, including sample playback offset/end, the verified layer squared velocity curve, per-zone `SampleOsc.Level` gain offset, zone `Amp.Pan` pan mapping, key/velocity crossfade assignment through HALion `Amp.Crossfade*` edge handles, static pitch-envelope assignment through HALion `Pitch.EnvAmount` plus `Pitch Env.EnvelopePoints`, static pitch-LFO assignment through HALion `LFO 1` plus a Pitch modulation row, and static filter-envelope assignment through `Filter.EnvAmount` plus `Filter Env.EnvelopePoints`. Unsupported or unverified ConvertWithMoss/SFZv2 behavior must stay marked unsupported and must warn or fail visibly instead of being silently ignored. The helper is SFZ converter support code, not a generic `halionbridge` Lua API.
...
Private or release-only converters are build-time drop-ins. A private converter can live under gitignored `converters/private/<name>`, be listed in gitignored `converters.local.cmake`, or be passed through `HALIONBRIDGE_EXTRA_CONVERTER_DIRS`. Drop-ins are statically linked into the CLI and must call `halionbridge_register_converter_target(TARGET ... REGISTRATION_FUNCTION ... HEADER ...)` from their CMake. Registration data is collected through CMake global properties so converter targets registered from nested `add_subdirectory()` scopes are included in the generated registration source. Runtime converter DLL/plugin loading is intentionally not used because it would add ABI, signing, notarization, and deployment failure modes. `ConverterSourcePathKind` declares whether a converter accepts file input, directory input, or both.

## VSTPreset Remap Mode

`halionbridge remap-vstpresets` is a runtime command, not a converter setup command. It scans an input directory recursively for `.vstpreset` files, copies them into a temporary UUID-named directory below HALion's user preset root, embeds the relative preset paths into the generated `halionbridge_runtime.lua` module, stages an embedded `halionbridge_preset_remap.lua` module beside it, and runs HALion through the existing bootstrap preset and marker wait loop. The Lua module loads each staged preset, calls `findZones(true)`, rewrites matching `SampleOsc.Filename` prefixes, saves changed staged presets through HALion's `savePreset()`, and writes the normal OK/failed marker presets in the staging directory. Only after a successful terminal marker does C++ copy the listed `.vstpreset` files to the requested output directory.
...
`export` scans the input directory for `.vstpreset` files and writes a UTF-8 CSV containing `filename_path`, `target_preset_name`, and the supported metadata columns: `MediaAuthor`, `MediaLibraryManufacturerName`, `MediaLibraryName`, `MediaComment`, `MediaRating`, `MusicalArticulations`, `MusicalCategory`, `MusicalInstrument`, `MusicalMoods`, `MusicalProperties`, `MusicalStyle`, `MusicalSubStyle`, and `VST3UnitTypePath`. Existing CSV files are refused unless `--overwrite` is supplied. `apply` reads that CSV as UTF-8, matches rows by normalized forward-slash `filename_path`, derives the output filename stem from `target_preset_name`, removes filename characters that are not portable across Windows and macOS, and writes updated preset copies to an empty or missing output directory. Successful export/apply runs return an info diagnostic that the CLI prints under the version header with the processed preset count and output path. Scanning is top-level only unless `--recursive` is supplied. Apply mode is strict: duplicate paths, unsafe relative paths, target names that sanitize to no usable filename, case-insensitive source or sanitized output path collisions, unmatched CSV rows, scanned presets missing rows, invalid containers, and invalid metadata XML fail before or during output generation.

The metadata writer treats present CSV metadata columns as authoritative. Non-empty cells set or update the matching XML attribute. Empty cells remove the matching attribute. Metadata columns omitted from the CSV are left untouched. Unknown XML attributes such as plugin identity fields are preserved, and unknown CSV columns are ignored on apply for compatibility with older or hand-annotated CSVs. Output preset files and CSV files are written through temporary files in the target directory and renamed into place only after a complete successful write.
...
`DEBUG=1` is enabled for `Debug` builds so JUCE and the first-party warning-as-error gate agree on the build mode. The JUCE diagnostic definitions are enabled automatically for `Debug` builds and explicitly for presets that set `HALIONBRIDGE_ENABLE_DIAGNOSTICS=ON`.
...
- halionbridge is a single-purpose builder host. Users call `halionbridge build <build-directory>`. The directory must contain `halionbridge_build.lua`; build script modules may create and save program presets, layer presets, or other HALion artifacts independently. Users can pass `--output-directory <dir>` to write generated `.vstpreset` files outside the Lua build directory. Users can run `halionbridge init <build-directory>` to generate a simple sorted `halionbridge_build.lua` from top-level Lua files before running the build.
...
- The compile-time assets are `halion-lua/builder_bootstrap.vstpreset`, `halion-lua/builder.lua`, and `halion-lua/preset_remap.lua`, embedded through the `halionbridge_assets` binary-data target. The checked-in `halion-lua/builder_bootstrap.lua` is the canonical source text for the inline Lua saved inside `builder_bootstrap.vstpreset`; it should call `require("halionbridge_runtime")`.
- For builder runs, the bridge writes temporary `halionbridge_runtime.lua` and `halionbridge_builder.lua` files into the HALion user scripts directory under `Documents/Steinberg/HALion/Library/scripts`. HALion Lua does not reliably expose the host process environment or current working directory to `require()`, so this generated runtime module is the authoritative handoff. It sets the Lua global `HALIONBRIDGE_RUNTIME_ROOT`, optional `HALIONBRIDGE_OUTPUT_ROOT`, optional build-slice globals, prepends the build directory to `package.path` if needed, clears cached runtime/builder modules, and then loads the temporary embedded builder module. Previous files with the same names are restored after the run; otherwise the temporary files are deleted. Because these filenames and the working directory/environment handoff are shared process/user state, halionbridge acquires a fail-fast inter-process lock for the whole runtime-staging lifetime and rejects overlapping runs.
- For preset-remap runs, the bridge writes temporary `halionbridge_runtime.lua` and `halionbridge_preset_remap.lua` files into the same HALion user script directory under the same lock. The generated runtime module sets `HALIONBRIDGE_PRESET_REMAP_ROOT`, `HALIONBRIDGE_PRESET_REMAP_PRESETS`, `HALIONBRIDGE_PRESET_REMAP_OLD_ROOT`, `HALIONBRIDGE_PRESET_REMAP_NEW_ROOT`, and `HALIONBRIDGE_PRESET_REMAP_PLUGIN_CODE`, prepends the staging directory to `package.path`, clears cached runtime/remap modules, and loads the embedded remap module. The preset list is embedded as Lua data because HALion Lua may reject `io.open`/`io.lines` reads in preset staging directories. The normal processing loop can watch the staging directory because the remap module writes the same terminal and progress marker filenames as the generic builder.
- Running `halionbridge` without arguments is equivalent to `halionbridge --help` and exits successfully after printing usage.
- For HALion batch builds, `halion-lua/builder.lua` is a neutral build script runner. It loads build script modules listed in `halionbridge_build.lua`, passes each module a documented `ctx` API, forwards file-level progress through temporary `hbp_*.vstpreset` marker presets, aggregates build script results, and writes `halionbridge_status_ok.vstpreset` or `halionbridge_status_failed.vstpreset` in the build directory using HALion's `savePreset()` API. `ctx.output_dir` equals the build directory unless `--output-directory <dir>` is supplied; `ctx.save_preset()` redirects relative paths and paths under `ctx.script_dir` into `ctx.output_dir` while status/progress markers remain in the build directory. Before running the batch, the builder raises HALion's controller script execution timeout to 600000 ms with `setScriptExecTimeOut()` when the API is available, then restores the previous value after the final status-marker write attempt even if marker writing fails. This is used instead of HALion `wait()`, because `wait()` is only valid inside callbacks while build script entrypoints run as controller global/module code. For statically parseable build files, the CLI parent runs normal headless chunks in hidden `--halionbridge-build-worker` child processes, up to 1000 scripts per worker by default. The parent polls the build directory for progress markers while the worker is running, so progress can be printed even when the worker is blocked inside HALion, provided HALion has actually flushed the marker file to disk. The parent also emits a 5-second build-worker heartbeat for long chunks so large synchronous HALion invocations do not look stalled. `--build-chunk-size <n>` changes the chunk size, and `--fail-fast` stops after the first failed chunk. Lua build failures and per-chunk timeouts are recorded but later chunks continue by default, with a final nonzero exit if any chunk failed; setup, plugin, preset-apply, stop, and cleanup failures stop immediately. GUI runs, `--nokill` inspection runs, embedded callers without `AppOptions::executableFile`, and dynamic/unparseable build files run in-process. The bridge deletes stale status/progress files before applying the preset, deletes each consumed progress marker immediately after logging it, drains late progress markers when a terminal status marker appears, and performs one final progress-marker sweep after releasing HALion plugin resources. Successful OK status markers are deleted; failed status markers remain after failed runs for diagnostics. Because current build script entrypoints run as synchronous HALion global/module code, marker visibility still depends on HALion's `savePreset()` behavior; if HALion defers marker file creation internally, the host will still observe those markers in batches.
- Each build worker writes its final `RunResult` code to a parent-provided temporary result file before exiting. The parent reads that file after worker output is drained, falling back only to an explicitly nonzero process exit code if the result file is unavailable. This keeps plugin-load, preset-apply, setup, timeout, and cleanup failures from being misreported as successful chunks on POSIX child-process implementations where exit-code collection is separate from output pipe completion.
- The CLI installs Ctrl+C/termination handlers before init, convert, scan-worker, build-worker, and build commands. Handlers set a shared stop flag. SFZ conversion checks the flag during directory traversal, between file conversions, and before writing generated files. In normal headless builds, the parent process does not load HALion directly; it supervises the active worker, lets it exit for up to 5 seconds after Ctrl+C, then kills it if HALion is still stuck and returns `RunResult::stopped`. A forced worker kill may skip worker-side marker cleanup, so stale marker cleanup on the next run remains part of the recovery path. GUI and `--nokill` inspection runs stay cooperative and in-process so HALion can release through normal C++ cleanup. Preset-remap staging cleanup is synchronous and guarded; after successful copy-back it performs one recursive delete and reports a simple warning when deletion remains blocked.
...
- The default build timeout is 3600 seconds. Use `--timeout-seconds <n>` to choose another finite timeout, or `--no-timeout` / `--timeout-seconds 0` only for explicit manual inspection runs that should wait indefinitely.
