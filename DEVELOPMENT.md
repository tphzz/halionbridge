...
The SFZ converter-owned helper module lives at `converters/sfz/lua/halionbridge-sfz.lua` and is embedded into the converter at build time. Converted SFZ output directories include it as a helper module named `halionbridge-sfz.lua`; it is not listed in `halionbridge_build.lua`. Helper version `1` exposes conservative capabilities and defensive HALion assignment helpers for the fields already covered by the current converter output. Unsupported or unverified ConvertWithMoss/SFZv2 behavior must stay marked unsupported and must warn or fail visibly instead of being silently ignored. The helper is SFZ converter support code, not a generic `halionbridge` Lua API.

The SFZ converter uses `sfz::Synth::loadSfzFile()` as the primary region model, but it also runs sfizz's parser listener over the same source file to recover whether loop opcodes were explicitly written. This is necessary because sfizz's resolved model can replace `loop_start=0` with a WAV `smpl` chunk loop start: `0` is both a valid SFZ value and sfizz's default sentinel. Explicit source loop values must win over audio-file metadata so converted HALion presets match the SFZ author's text. Generated Lua writes `SampleOsc.Filename`, root key, loop parameters, and optional tone parameters before writing `zone.keyLow`, `zone.keyHigh`, `zone.velLow`, and `zone.velHigh`, because loading a sample can populate HALion fields from sampler metadata.
