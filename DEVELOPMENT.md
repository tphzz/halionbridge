...
Static pitch LFO conversion covers one probe-verified simple pitch-targeted LFO per region. Numbered SFZv2 opcodes `lfoNN_pitch`, `lfoNN_freq`, `lfoNN_phase`, `lfoNN_delay`, and `lfoNN_fade` are preferred, while legacy `pitchlfo_depth`, `pitchlfo_freq`, `pitchlfo_phase`, `pitchlfo_delay`, and `pitchlfo_fade` use the same HALion mapping. Depth is clamped to `-1200..1200` cents and mapped to HALion modulation `Destination.Depth = pitch_depth / 20`; frequency maps directly to `LFO 1.Rate`, phase maps directly to `LFO 1.InitPhase = phase * 360`, and delay/fade seconds map to `LFO 1.Delay`/`LFO 1.FadeIn` milliseconds. Generated Lua sets `LFO 1.Trigger=1` so note starts retrigger the phase, then assigns `ModulationSource.lfo1` to a Pitch modulation row as a required operation. Multiple pitch LFOs, non-sine or stepped LFO behavior, sync variants, dynamic CC/velocity mappings, and non-pitch LFO targets are reported as `unsupported-pitch-lfo` instead of being silently approximated. Probes `071` through `073` are the manual validation chain for this mapping.
