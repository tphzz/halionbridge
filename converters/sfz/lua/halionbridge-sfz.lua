...
local function halion_sustain_loop_mode(loop_mode)
    if loop_mode == nil then
        return 1
    end

    if type(loop_mode) == "number" then
        return loop_mode
    end

    if loop_mode == "continuous" or loop_mode == "loop_continuous" then
        return 1
    end

    if loop_mode == "sustain" or loop_mode == "until_release" or loop_mode == "loop_sustain" then
        return 4
    end
...
function hb.apply_optional_sample_fields(ctx, zone, region)
    ...
    local sample_natural_end = type(playback) == "table" and playback.natural_end or region and region.sample_natural_end
    if sample_end then
    ...
    if loop_start and loop_end then
        ...
        local loop_mode = type(loop) == "table" and loop.mode or region and region.loop_mode
        -- SFZ loop_end is the last sample played in the loop. HALion's loop end
        -- field is a marker position, so a 200-sample cycle with SFZ
        -- loop_end=199 must be written as loop end marker 200. Probe 041 showed
        -- that SampleEnd must not be truncated to the loop end for looped
        -- samples with post-loop tails; only explicit SFZ end=... writes the
        -- sample end marker above.
        hb.set_parameter_if_available(ctx, zone, "SampleOsc.PlaybackMode", 0)
        hb.set_parameter_if_available(ctx, zone, "SampleOsc.LoopSelect", 0)
        hb.set_parameter_if_available(ctx, zone, "SampleOsc.SustainLoopModeA", halion_sustain_loop_mode(loop_mode))
    ...
    local filter_type = type(filter) == "table" and filter.type or region and region.filter_type
    if filter_type then
        hb.set_parameter_if_available(ctx, zone, "Filter.Type", filter_type)
    end
    local filter_mode = type(filter) == "table" and filter.mode or region and region.filter_mode
    if filter_mode then
        hb.set_parameter_if_available(ctx, zone, "Filter.Mode", filter_mode)
    end
    local filter_shape_a = type(filter) == "table" and filter.shape_a or region and region.filter_shape_a
    if filter_shape_a then
        hb.set_parameter_if_available(ctx, zone, "Filter.ShapeA", filter_shape_a)
    end
    local filter_shape_b = type(filter) == "table" and filter.shape_b or region and region.filter_shape_b
    if filter_shape_b then
        hb.set_parameter_if_available(ctx, zone, "Filter.ShapeB", filter_shape_b)
    end
    ...
    local filter_resonance = type(filter) == "table" and filter.resonance or region and region.filter_resonance
    if filter_resonance then
        hb.set_parameter_if_available(ctx, zone, "Filter.Resonance", filter_resonance)
    end
