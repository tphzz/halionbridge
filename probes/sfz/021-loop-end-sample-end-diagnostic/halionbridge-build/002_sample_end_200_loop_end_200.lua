...
return function(ctx)
    local layer, layerErr = hb.create_layer(ctx, "probe_sample_end_200_loop_end_200")
    ...
    hb.set_parameter_if_available(ctx, zone, "SampleOsc.SampleEnd", 200)
    ...
    local saved, saveErr = hb.save_layer_preset(ctx, layer, "probe_sample_end_200_loop_end_200.vstpreset")
    if not saved then return hb.fail(saveErr) end
    return hb.ok("Built probe_sample_end_200_loop_end_200.vstpreset", 1)
end
