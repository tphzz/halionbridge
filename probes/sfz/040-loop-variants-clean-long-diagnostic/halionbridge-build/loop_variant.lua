...
local probeRoot = "C:/develop/halionbridge/probes/sfz/040-loop-variants-clean-long-diagnostic/source/samples/"
...
return function(config)
    ...
    local region = {
        name = layerName,
        sample_playback = {
            sample = probeRoot .. "clean_loop_with_tail.wav",
        },
        mapping = {
            key_low = 57,
            key_high = 57,
    ...
    return function(ctx)
        ...
        if config.loop_mode ~= nil then
            ...
            hb.set_parameter_if_available(ctx, zone, "SampleOsc.SustainLoopStartA", 11000)
            hb.set_parameter_if_available(ctx, zone, "SampleOsc.SustainLoopEndA", 22000)
        end
