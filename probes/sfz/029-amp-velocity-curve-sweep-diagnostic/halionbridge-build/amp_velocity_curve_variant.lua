...
local sampleFile = "C:/develop/halionbridge/probes/sfz/029-amp-velocity-curve-sweep-diagnostic/source/samples/sine_A3_single_cycle.wav"
...
function M.build(ctx, curveIndex)
    local label = string.format("%03d Velocity Curve %d", curveIndex, curveIndex)
    local outputFile = string.format("%03d_velocity_curve_%d.vstpreset", curveIndex, curveIndex)
    ...
    hb.set_parameter_if_available(ctx, zone, "Amp Env.VelocityToLevelCurve", curveIndex)
