...
bool isPresetRemapStageDirectory(const std::filesystem::path& stageDirectory, const std::filesystem::path& expectedUserPresetRoot)
{
    constexpr const char* kPresetRemapTemporaryDirectoryPrefix = "halionbridge-remap-";

    if (stageDirectory.empty() || expectedUserPresetRoot.empty())
        return false;

    const auto name = stageDirectory.filename().string();
    if (name.rfind(kPresetRemapTemporaryDirectoryPrefix, 0) != 0)
        return false;

    return comparableAbsolutePath(stageDirectory.parent_path()) == comparableAbsolutePath(expectedUserPresetRoot);
}
...
std::string createPresetRemapRuntimeModuleText(const PresetRemapRuntimeConfig& config)
{
    ...
    text << "-- Generated by halionbridge.exe for the embedded HALion preset-remap bootstrap.\n"
         ...
         << "HALIONBRIDGE_PRESET_REMAP_PRESETS = {\n";
    for (const auto& relativePath : config.relativePresetPaths)
        text << "    " << quoteForLua(relativePath) << ",\n";
    text << "}\n"
                                      ...
                                      std::string& errorMessage)
{
    ...
    const auto stageStatus = std::filesystem::symlink_status(stageDirectory, statusError);
    if (statusError)
    {
        if (isMissingPathError(statusError))
            return true;

        errorMessage = "Could not inspect temporary preset-remap directory " + stageDirectory.string() + ": " + statusError.message();
        return false;
    }

    if (stageStatus.type() == std::filesystem::file_type::not_found)
        return true;

    if (std::filesystem::is_symlink(stageStatus) || !std::filesystem::is_directory(stageStatus))
    {
        errorMessage = "Refusing to clean a non-directory or symlinked remap staging root: " + stageDirectory.string();
        return false;
    }

    if (!isPresetRemapStageDirectory(stageDirectory, expectedUserPresetRoot))
    {
        errorMessage = "Refusing to clean path outside the expected halionbridge remap staging area: " + stageDirectory.string();
        return false;
    }

    std::error_code removeError;
    std::filesystem::remove_all(stageDirectory, removeError);
    if (removeError)
    {
        if (isMissingPathError(removeError))
            return true;

        errorMessage = "Could not delete temporary preset-remap directory " + stageDirectory.string() + ": " + removeError.message();
        return false;
    }
